...
void main(void)
{
	...
	MUSART1_voidInit();							/* BR 9600 EN--(RX,TX) */

	MSTK_voidInit();

	MSTK_voidSetIntervalSingle(15000000,func);  /* Sleep To 15 Sec After Call Func */
	...
	while(u8TimeOutFlag == 0)
	{
		...
		if (Local_u8RecStatus == 1)					/*	 If You Receive 	*/
		{
			...
			if(u8RecBuffer[u8RecCounter] == '\n')	/* Wait Untile The End Of Buffer */
			{
				...
				if (u8BLWriteReq == 1)
				{
					FPEC_voidEraseAppArea();			// It Delet From Page 4 To 64
					u8BLWriteReq = 0;					// It Will Be Ture Frist Time Only
				}
				
				/* Parse */
				Parser_voidParseRecord(u8RecBuffer);		// Parsing one record of the flash
